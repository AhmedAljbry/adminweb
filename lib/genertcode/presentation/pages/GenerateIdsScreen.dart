// lib/genertcode/presentation/pages/GenerateIdsScreen.dart
...
class GenerateIdsScreen extends StatefulWidget {
  ...
  const GenerateIdsScreen({super.key, required this.configController});

  @override
  State<GenerateIdsScreen> createState() => _GenerateIdsScreenState();
}

class _GenerateIdsScreenState extends State<GenerateIdsScreen> {
  static const _TAG = 'GenerateIdsScreen';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<GenCodeBloc>().add(const StartConnectivityWatch());
    });
  }
  ...
  void _maybeSyncFormWithConfig(AppConfig cfg, {required bool isBusy}) {
    ...
    if (_collectionCtrl.text != cfg.collection) _collectionCtrl.text = cfg.collection;
    if (_fileCtrl.text != cfg.file)             _fileCtrl.text = cfg.file;
    if (_documentCtrl.text != cfg.document)     _documentCtrl.text = cfg.document;
  }
          ...
          return Theme(
            ...
            child: BlocConsumer<GenCodeBloc, GenCodeState>(
              listener: (context, state) async {
                _log('Bloc listener: batchState=${state.batchState} '
                    'isRunning=${state.isRunning} '
                    'fs=${state.firestoreProgress} ex=${state.excelProgress} '
                    'msg=${state.message}');
                ...
                // ✅ نجاح – نظهر Dialog فيه مسار الحفظ
                  ...
                  await showDialog(
                    ...
                    builder: (_) => AlertDialog(
                      ...
                      content: Column(
                        ...
                        children: [
                          Text('عدد المطلوب: ${result.requestedCount}'),
                          Text('تم الرفع إلى Firestore: ${result.uploadedToFirestore}'),
                ...
                Future<void> _onStop() async {
                  final ok = await showDialog<bool>(
                    ...
                    builder: (_) => AlertDialog(
                      ...
                      content: const Text('هل تريد إيقاف العملية الجارية؟'),
                      actions: [
                        TextButton(
                          onPressed: () => Navigator.pop(context, false),
                          child: const Text('إلغاء'),
                    ...
                    child: Center(
                      child: ConstrainedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            _BrandLogo(url: cfg.logoUrl, key: ValueKey(cfg.logoUrl)),
                            ...
                            const Text(
                              'تأكد من اختيار الرقم بدقة. سيتم حفظ الملف في مجلد Download/code ورفعه إلى Firestore.',
                              textAlign: TextAlign.center,
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: OutlinedButton.icon(
                                    onPressed: state.isRunning ? _onStop : null,
                                    ...
                                    style: OutlinedButton.styleFrom(
                                      ...
                                      foregroundColor:
                                      state.isRunning ? Colors.red : Colors.grey,
                                    ),
                                  ),
                                ),
                              ],
...
class _ProgressCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final percent = (v * 100).toStringAsFixed(1);
    return Card(
